...
<body>
  ...
  <?php
	...
	$response = $traccar->positions($sessionId,$deviceid,$data_ini,$data_final);
	...
	foreach($positions as $position){
		...
		$data = formata_data($position->serverTime,4);
		$data = '<br><b>Data: </b>'.$data.'<br><b>Endereço: </b>'.$position->address.'<br> Lat: '.$position->latitude.' - Log: '.$position->longitude;
		
		$var_dados .= '["'.$data.'"]';
		$i++;
	}	
	if ( $i == 0 ){
		echo "<div id='map'><script src='https://unpkg.com/leaflet/dist/leaflet.js'></script>
<script>

  var map = L.map('map').setView([ -15.7797200 ,-47.9297200 ], 5); // Porto Alegre coordinates
...
";
		echo '<script> alert("Sem dados para exibir"); </script>';
	}	
  ?>
...
<script>
    ...
    if (lastIndex < points.length) {
	  ...
	  if( lastIndex == 0){
		...
		if (lastIndex+1 ==  points.length){
			...
			var marker = L.marker(point, { icon: L.icon({ iconUrl: 'img/pin.png', iconSize: [40, 40] }) }).addTo(map);  
		}	
	  }	  
  ...
  function togglePause() {
   ...
   function toggleAutoFast() {
    intervalId = setInterval(addPoint, 200); // Chamando addPoint a cada segundo
	document.getElementById('pause').disabled = false;
	document.getElementById('addBtn').disabled = true;
	document.getElementById('removeBtn').disabled = true;
	document.getElementById('autoBtn').disabled = true;
