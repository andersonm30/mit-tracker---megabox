...
//public static $host='http://128.140.104.102';
public static $host='localhost';
private static $adminEmail='admnistrador';
private static $adminPassword='$uporte80';
...
public static function userAdd($sessionId,$name,$email,$password,$attributes){
	...
	$data='{"id":"'.$id.'","name":"'.$name.'","email":"'.$email.'","readonly":"'.$readOnly.'","administrator":"'.$administrator.'","map":"'.$map.'","latitude":"'.$latitude.'","longitude":"'.$longitude.'","zoom":"'.$zoom.'","password":"'.$password.'","coordinateFormat":"'.$coordinateFormat.'","disabled":"'.$disabled.'","expirationTime":"'.$expirationTime.'","deviceLimit":"'.$deviceLimit.'","userLimit":"'.$userLimit.'","deviceReadonly":"'.$deviceReadonly.'","limitCommands":"'.$limitCommands.'","token":"'.$token.'","attributes":'.$attributes.'}';
...
//Positions
public static function positions($sessionId,$deviceId,$from,$to){
...
public static function curl($task,$method,$cookie,$data,$header) {
	...
	if($method=='POST' || $method=='PUT' || $method=='DELETE') {
		...
		curl_setopt( $ch, CURLOPT_HTTPHEADER, array('Authorization: Basic ' . $_SESSION['Basic'],"Cookie: ".$cookie ));
	}		
