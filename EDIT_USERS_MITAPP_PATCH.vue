      ...
      <div class="users-footer users-footer--expanded">
        ...
        <div class="footer-group footer-group--extras">
          <el-button
            ...
            :disabled="selected === 0"
            @click="createSession"
            @mouseenter.stop="showTip($event, KT('user.changesesion') || 'Criar Sessão')"
            @mouseleave="hideTip"
          >
            <i class="fas fa-passport"></i>
            <span class="btn-text">{{ KT('user.changesesion') || 'Sessão' }}</span>
          </el-button>
...
const getUserDeviceCount = (userId) => {
  ...
  return deviceCounts.value[userId] ?? '-'
}

const getUserUserCount = (userId) => {
  if (loadingCounts.value[userId]) return '...'
  return userCounts.value[userId] ?? '-'
}
...
// MITAPP: Carregar contadores e invoices ao abrir modal
const handleModalOpened = async () => {
  // Carregar contadores dos primeiros usuários visíveis (otimização)
  const visibleUsers = filteredUsersFinal.value.slice(0, 20)
  
  for (const user of visibleUsers) {
    if (!deviceCounts.value[user.id] && !loadingCounts.value[user.id]) {
      refreshUserCounts(user.id)
    }
  }
  
  // Carregar invoices se módulo estiver ativo
...
const loadUserInvoices = async () => {
  ...
  for (const user of users) {
    try {
      const invoices = await store.dispatch('invoices/getUserInvoices', user.id).catch(() => [])
...
/* ===== MITAPP: COLUNAS COM CONTADORES CLICÁVEIS ===== */
.clickable-count {
  display: inline-flex;
  align-items: center;
  gap: 7px;
