<template>
  ...
  <div style="border: silver 1px solid; border-radius: 5px;margin-top: 20px;height: calc(100vh - 200px);">
    <div class="deviceHead">
      ...
      <div class="name" style="font-size: 12px;font-weight: 100;padding: 5px;flex: 2;">Grupo Pai</div>
    </div>
    <div style="overflow-x: hidden;overflow-y: scroll;height: calc(100vh - 230px);">
      <template v-if="store.getters.advancedPermissions(49)">
      <div v-for="(group) in filteredDevices" :key="group.id" @click="editGroupRef.editGroup(group)" @contextmenu.prevent="actContext($event,group)" class="device">
        <div class="name" >{{group.name}}</div>
        <div  class="name" style="flex: 2;">
          {{store.getters['groups/getGroupNameById'](group.groupId)}}
        </div>
      </div>
      </template>
      <template v-else>
        <div v-for="(group) in filteredDevices" :key="group.id"  @contextmenu.prevent="actContext($event,group)" class="device">
          <div class="name" >{{group.name}}</div>
          <div  class="name" style="flex: 2;">
            {{store.getters['groups/getGroupNameById'](group.groupId)}}
          </div>
...
const actContext = (e,group)=>{
  ...
  tmp.push({
    text: KT('group.edit'), cb: () => {
      editGroupRef.value.editGroup(group)
    }
  });
  ...
  tmp.push({
    text: KT('geofence.geofences'), cb: () => {
      linkObjectsRef.value.showObjects({groupId: group.id, type: 'geofences'});
    }
  });

  tmp.push({
    text: KT('attribute.computedAttributes'), cb: () => {
      linkObjectsRef.value.showObjects({groupId: group.id, type: 'attributes'});
    }
  });

  tmp.push({
    text: KT('driver.drivers'), cb: () => {
      linkObjectsRef.value.showObjects({groupId: group.id, type: 'drivers'});
    }
  });

  tmp.push({
    text: KT('command.savedCommands'), cb: () => {
      linkObjectsRef.value.showObjects({groupId: group.id, type: 'commands'});
    }
  });

  tmp.push({
    text: KT('notification.notifications'), cb: () => {
      linkObjectsRef.value.showObjects({groupId: group.id, type: 'notifications'});
    }
  });

  tmp.push({
    text: KT('maintenance.maintenances'), cb: () => {
      linkObjectsRef.value.showObjects({groupId: group.id, type: 'maintence'});
    }
  });
...
const filteredDevices = computed(()=>{
  if(query.value === ''){
    return store.state.groups.groupList;
  }else {
    return store.state.groups.groupList.filter((d) => {
