...
! function(e, t) {
    ...
    var t = L.MarkerClusterGroup = L.FeatureGroup.extend({
        ...
        _removeLayer: function(e, t, i) {
            // Declare variables for grid clusters, unclustered grid, feature group, and map
            var r = this._gridClusters,
                n = this._gridUnclustered,
                s = this._featureGroup,
                o = this._map,
                a = Math.floor(this._map.getMinZoom());  // Minimum zoom level
        
            // If t is true, remove from the grid of unclustered markers
            if (t) {
                this._removeFromGridUnclustered(e, this._maxZoom);
            }
        
            var h, 
                l = e.__parent,  // Parent of the marker
                u = l._markers;  // Markers array of the parent
        
            // Remove the marker from its parent's markers array
            this._arraySplice(u, e);
        
            // Traverse up through parent clusters
            while (l) {
                // Decrease child count and mark bounds for update
                l._childCount--;
                l._boundsNeedUpdate = true;
        
                // If zoom level is smaller than the minimum zoom, stop the loop
                if (l._zoom < a) break;
        
                // If the parent has only one child left, handle the cluster update
                if (t && l._childCount <= 1) {
                    // Get the other marker from the cluster
                    h = l._markers[0] === e ? l._markers[1] : l._markers[0];
        
                    // Remove the current cluster and add the remaining marker to the grid
                    ...
                    // If the icon exists, remove the old cluster and add the new marker
                    if (l._icon) {
                        s.removeLayer(l);
                        if (!i) s.addLayer(h);
                    }
                } else {
                    // If not the last child, mark the icon as needing an update
            ...
            // Delete the parent reference from the marker
            delete e.__parent;
        },
        ...
        _moveEnd: function() {
            // 🛡️ GUARD: Evita crash se _map for null durante animações/transições
            if (!this._map) return;
        ...
        _generateInitialClusters: function() {
            // 🛡️ GUARD: Evita crash se _map for null
            if (!this._map) return;
        ...
        _getExpandedVisibleBounds: function() {
            // 🛡️ GUARD: Evita crash se _map for null (durante animações/transições)
            if (!this._map) {
                return this._mapBoundsInfinite || L.latLngBounds([[-90, -180], [90, 180]]);
            }
    ...
    L.Marker.include({
        ...
        _sqDist: function(e, t) {
            ...
            if (!e || typeof e.x === 'undefined' || typeof e.y === 'undefined' ||
                !t || typeof t.x === 'undefined' || typeof t.y === 'undefined') {
                console.warn('Coordenadas inválidas detectadas:', { e, t });
                return Infinity; // Retorna um valor padrão que indica erro ou distância infinita
            }
