  ...
  <el-dialog :lock-scroll="true" :top="'50px'" v-model="show">
    ...
    <template v-slot:title>
      <div  style="border-bottom: #e0e0e0 1px solid;padding: 20px;">
        <div class="modal-title" >{{KT('notification.title','Notificações')}}</div>
      </div>
    </template>
    ...
    <div class="itm" style="display: flex;background: #eeeeee;">
      <div style="width: 30px;padding: 10px;font-size: 12px;">{{KT('notification.id')}}</div>
      ...
      <div style="flex: 1;padding: 10px;font-size: 12px;text-align: center;">{{KT('notification.alarms')}}</div>
    ...
    <div style="height: calc(100vh - 300px); overflow: hidden; overflow-y: scroll;">
      ...
      <div class="itm" v-for="(u,k) in store.state.events.eventsList" :key="k" @click="selected = (selected!==u.id)?u.id:0" :class="{tr1: (k%2),tr2: !(k%2),selected: (selected === u.id)}" style="display: flex;">

<div style="width: 30px;padding: 10px;font-size: 14px;">{{u.id}}</div>
        <div style="width: 140px;padding: 10px;font-size: 14px;">{{KT('notification.types.'+u.type)}}</div>
        <div style="width: 100px;padding: 10px;font-size: 14px;text-align: center;">{{KT((u.always)?'yes':'no')}}</div>
