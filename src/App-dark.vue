...
const userMenu = (e) => {
  ...
  if (!store.state.auth.attributes['isShared']) {
    ...
    if (store.getters.advancedPermissions(16)) {
      tmp.push({ text: KT('usermenu.users'), icon: 'fas fa-users', cb: () => editUsersRef.value.showUsers() })
    }
    ...
    if (store.getters.isAdmin) {
      tmp.push({ text: KT('usermenu.server'), icon: 'fas fa-server', cb: () => editServerRef.value.showServer() })
    }

    if (store.getters.advancedPermissions(32)) {
      tmp.push({ text: KT('usermenu.notifications'), icon: 'fas fa-bell', cb: () => editNotificationsRef.value.showNotifications() })
    }

    if (store.getters.advancedPermissions(80)) {
      tmp.push({ text: KT('usermenu.drivers'), icon: 'fas fa-id-card', cb: () => editDriversRef.value.showDrivers() })
    }

    if (store.getters.advancedPermissions(88)) {
      tmp.push({ text: KT('usermenu.calendars'), icon: 'fas fa-calendar-alt', cb: () => editCalendarsRef.value.showCalendars() })
    }

    if (store.getters.advancedPermissions(96)) {
      tmp.push({ text: KT('usermenu.maintenance'), icon: 'fas fa-tools', cb: () => editMaintenancesRef.value.showMaintenances() })
    }
  }
